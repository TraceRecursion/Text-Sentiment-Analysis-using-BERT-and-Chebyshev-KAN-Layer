...
5. [Seven-Class Sentiment Analysis](#seven-class-sentiment-analysis)
    - [Dataset](#dataset)
...
1. **数据清理**：去除无效字符和噪音，如 HTML 标签和特殊符号。
2. **Tokenization**: Using BERT's tokenizer to preprocess the text.
2. **标记化**：使用 BERT 的标记器预处理文本。
3. **Label Mapping**: Converting sentiment labels (positive, negative, neutral) into numerical form for model training and prediction.
3. **标签映射**：将情感标签（积极、消极、中性）转换为数字形式以进行模型训练和预测。
4. **Data Splitting**: Dividing the dataset into training (70%) and testing (30%) sets.
4. **数据划分**：将数据集分为训练集（70%）和测试集（30%）。
